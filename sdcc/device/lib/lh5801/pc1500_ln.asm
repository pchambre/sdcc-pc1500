;--------------------------------------------------------------------------
; pc1500_ln.asm - PC-1500 native LN (single-operand, mutate-in-
; place) math routine, for the LH5801 SDCC port.
;
; Paul Chambre, 2026
;
; ROM address 0xF161 (per the PC-2 Assembly Language manual's memory
...
;--------------------------------------------------------------------------
	...
	.globl _pc1500Ln
	.globl _pc1500Ln_PARM_1
	.globl __pc1500_fp_copyin
	.globl __pc1500_fp_copyout

;-----------------------------------------------------------------
; void pc1500Ln(pc1500_float_t v);
;-----------------------------------------------------------------
_pc1500Ln:
	lda	(_pc1500Ln_PARM_1)
	sta	xh
	lda	(_pc1500Ln_PARM_1 + 1)
	sta	xl
