;--------------------------------------------------------------------------
; pc1500_sgn.asm - PC-1500 native SGN (single-operand, mutate-in-
; place) math routine, for the LH5801 SDCC port.
;
; Paul Chambre, 2026
;
; ROM address 0xF59D (per the PC-2 Assembly Language manual's memory
...
;--------------------------------------------------------------------------
	...
	.globl _pc1500Sgn
	.globl _pc1500Sgn_PARM_1
	.globl __pc1500_fp_copyin
	.globl __pc1500_fp_copyout

;-----------------------------------------------------------------
; void pc1500Sgn(pc1500_float_t v);
;-----------------------------------------------------------------
_pc1500Sgn:
	lda	(_pc1500Sgn_PARM_1)
	sta	xh
	lda	(_pc1500Sgn_PARM_1 + 1)
	sta	xl
