;--------------------------------------------------------------------------
; pc1500_log.asm - PC-1500 native LOG (single-operand, mutate-in-
; place) math routine, for the LH5801 SDCC port.
;
; Paul Chambre, 2026
;
; ROM address 0xF165 (per the PC-2 Assembly Language manual's memory
...
;--------------------------------------------------------------------------
	...
	.globl _pc1500Log
	.globl _pc1500Log_PARM_1
	.globl __pc1500_fp_copyin
	.globl __pc1500_fp_copyout

;-----------------------------------------------------------------
; void pc1500Log(pc1500_float_t v);
;-----------------------------------------------------------------
_pc1500Log:
	lda	(_pc1500Log_PARM_1)
	sta	xh
	lda	(_pc1500Log_PARM_1 + 1)
	sta	xl
