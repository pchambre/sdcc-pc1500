;--------------------------------------------------------------------------
; pc1500_acs.asm - PC-1500 native ACS (single-operand, mutate-in-
; place) math routine, for the LH5801 SDCC port.
;
; Paul Chambre, 2026
;
; ROM address 0xF492 (per the PC-2 Assembly Language manual's memory
...
;--------------------------------------------------------------------------
	...
	.globl _pc1500Acs
	.globl _pc1500Acs_PARM_1
	.globl __pc1500_fp_copyin
	.globl __pc1500_fp_copyout

;-----------------------------------------------------------------
; void pc1500Acs(pc1500_float_t v);
;-----------------------------------------------------------------
_pc1500Acs:
	lda	(_pc1500Acs_PARM_1)
	sta	xh
	lda	(_pc1500Acs_PARM_1 + 1)
	sta	xl
