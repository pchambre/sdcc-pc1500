;--------------------------------------------------------------------------
; pc1500_exp.asm - PC-1500 native EXP (single-operand, mutate-in-
; place) math routine, for the LH5801 SDCC port.
;
; Paul Chambre, 2026
;
; ROM address 0xF1CB (per the PC-2 Assembly Language manual's memory
...
;--------------------------------------------------------------------------
	...
	.globl _pc1500Exp
	.globl _pc1500Exp_PARM_1
	.globl __pc1500_fp_copyin
	.globl __pc1500_fp_copyout

;-----------------------------------------------------------------
; void pc1500Exp(pc1500_float_t v);
;-----------------------------------------------------------------
_pc1500Exp:
	lda	(_pc1500Exp_PARM_1)
	sta	xh
	lda	(_pc1500Exp_PARM_1 + 1)
	sta	xl
