;--------------------------------------------------------------------------
; pc1500_mul.asm - PC-1500 native two-operand floating-point multiplication, for
; the LH5801 SDCC port.
;
; Paul Chambre, 2026
;
; ROM address 0xF01A, "X * Y -> X" per the PC-2 Assembly Language
; manual, confirmed directly against ROM1.BIN this session (3.0 * 4.0 = 12.0,
...
;--------------------------------------------------------------------------
	...
	.globl _pc1500Mul
	.globl _pc1500Mul_PARM_1
	.globl _pc1500Mul_PARM_2
	.globl __pc1500_fp_copyin
	.globl __pc1500_fp_copyin2
	.globl __pc1500_fp_copyout

;-----------------------------------------------------------------
; void pc1500Mul(pc1500_float_t a, pc1500_float_t b);
;-----------------------------------------------------------------
_pc1500Mul:
	lda	(_pc1500Mul_PARM_1)
	sta	xh
	lda	(_pc1500Mul_PARM_1 + 1)
	sta	xl
	sjp	__pc1500_fp_copyin

	lda	(_pc1500Mul_PARM_2)
	sta	xh
	lda	(_pc1500Mul_PARM_2 + 1)
	sta	xl
